...
import { Send, Hash, UserPlus, Layout, MessageSquare, BarChart2, Trash2, Plus, Lock, LogIn, AlertCircle, Check, Clock, Loader2, Image as ImageIcon, X } from 'lucide-react';
...
export default function ModeratorApp() {
  ...
  const [members, setMembers] = useState<{id: string, username: string, displayName: string, tag: string}[]>([]);
  ...
  // FIX: Changed 'if: String' to 'id: string'
  ...
  useEffect(() => {
    const fetchMembers = async () => {
        if (activeGuildId && (window as any).electronAPI?.getMembers) {
            try {
                const memberList = await (window as any).electronAPI.getMembers(activeGuildId);
  ...
  useEffect(() => {
      const fetchChannels = async () => {
          if (activeGuildId && (window as any).electronAPI) {
              const chs = await (window as any).electronAPI.getChannels(activeGuildId);
              setChannels(chs);
  ...
  const handleLogin = async () => {
    ...
    try {
        ...
        if (success) {
            console.log('Login successful, fetching guilds...');
            const serverList = await (window as any).electronAPI.getGuilds();
            console.log('Fetched guilds:', serverList);
            
            if (serverList && serverList.length > 0) {
                setGuilds(serverList);
                setActiveGuildId(serverList[0].id);
                setIsLoggedIn(true);
            } else {
                setLoginError('Bot connected but no servers found. Please invite the bot to a server first.');
            }
        } else {
            setLoginError('Invalid Bot Token.');
        }
    } catch (e) {
        console.error('Login error:', e);
        setLoginError('Connection failed: ' + ((e as Error).message || 'Unknown error'));
    }
  ...
  const handleSendMessage = async () => {
    ...
    if (msgType === 'poll' && totalHours < 1) {
        setStatusMsg({type: 'error', text: 'Poll duration must be at least 1 hour.'});
        return;
    }
  ...
  return (
    <div className="flex h-screen bg-[#121212] font-sans text-red-50 overflow-hidden">
      ...
      <div className="w-64 bg-[#1e1e1e] flex flex-col border-r border-red-900/50 shrink-0">
        ...
        <div className="mt-auto p-4 border-t border-red-900/50 space-y-4">
            {/* NEW: Server Selector */}
            <div>
                <label className="text-xs font-bold text-red-300 uppercase mb-2 block">Select Server</label>
                <select 
                    value={activeGuildId}
            ...
            <div>
                ...
                <select 
                    ...
                    className="w-full bg-[#121212] p-2 rounded outline-none text-sm text-red-50 border border-red-900/30 focus:border-red-500 cursor-pointer"
                >
                    {channels.length === 0 && <option>No Text Channels</option>}
                    ...
                    <div className="max-w-xl space-y-6">
                        ...
                        {msgType === 'embed' && (
                            <div className="space-y-3 p-4 bg-[#1e1e1e] rounded border border-red-900/50">
                                <div>
                                    <label className="text-lg font-bold text-red-300 uppercase">Embed Title</label>
                                    <input 
                                        type="text" 
                                        value={embedTitle} onChange={(e) => setEmbedTitle(e.target.value)}
                                        className="w-full bg-[#121212] text-red-50 p-2 mt-1 rounded outline-none border border-red-900/30 focus:border-red-500 text-lg font-bold"
                                    />
                                </div>
                                <div>
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-1">
                                ...
                                >
                                    <UserPlus size={12} /> @ Mention
                                </button>
                            </div>
                            
                            {/* Mention Picker Dropdown */}
                            {showMentionPicker && (
                                <div className="mb-2 bg-[#1e1e1e] border border-red-900/50 rounded max-h-40 overflow-y-auto">
                                    <div className="p-2 text-xs text-red-300 border-b border-red-900/30">
                                        Select a member to mention:
                                    </div>
                                    {members.length === 0 ? (
                                        <div className="p-3 text-sm text-red-400 text-center">No members found</div>
                                    ) : (
                                        members.map(m => (
                                            <button
                                                key={m.id}
                                                onClick={() => {
                                                    insertMention(m.id, m.displayName);
                                                    setShowMentionPicker(false);
                                                }}
                                                className="w-full text-left px-3 py-2 hover:bg-red-900/20 text-sm text-red-100 flex items-center gap-2 border-b border-red-900/10 last:border-b-0"
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-red-400">@{m.username}</div>
                                                </div>
                                            </button>
                                        ))
                                    )}
                            ...
                            <textarea
                                ...
                                className="w-full bg-[#121212] text-red-50 p-3 rounded h-32 outline-none border border-red-900/30 focus:border-red-500 resize-none placeholder-red-300/50 disabled:opacity-50 disabled:cursor-not-allowed"
                            />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-red-300 uppercase">
                        ...
                        {msgType === 'poll' && (
                            <div className="space-y-4">
                                ...
                                {/* Days and Hours Selectors */}
                                <div>
                                    <label className="text-xs font-bold text-red-300 uppercase flex items-center gap-2 mb-2"><Clock size={14}/> Poll Duration</label>
                                    <div className="flex gap-4">
                                        <div className="flex-1">
                                            ...
                                            <select 
                                                ...
                                                className="w-full bg-[#121212] text-red-50 p-2 rounded border border-red-900/30 focus:border-red-500 cursor-pointer"
                                            >
                                                {[...Array(8).keys()].map(d => (
                    ...
                    <div className="max-w-xl space-y-6">
                         <div className="bg-[#1e1e1e] p-6 rounded-lg border border-red-900/50 shadow-lg shadow-red-900/10">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="text-xs font-bold text-red-300 uppercase">End Date & Time</label>
                                    <input 
                                        type="datetime-local" 
                                        value={prospectDate} 
                        ...
                        <div className="text-red-50 mt-1 whitespace-pre-wrap text-[15px] leading-snug">
                                ...
                                <div className="flex mt-1 flex-col gap-2">
                                    <div className="flex">
                                        ...
                                        <div className="bg-[#1e1e1e] rounded-r p-3 max-w-full w-full border border-red-900/50">
                                            <div className="font-bold text-white mb-1 text-xl">{embedTitle || 'Embed Title'}</div>
                            ...
                            {activeTab === 'message' && msgType === 'poll' && (
                                <div className="bg-[#1e1e1e] rounded p-3 mt-1 border border-red-900/50">
                                    ...
                                    <div className="mt-2 text-xs text-gray-500 border-t border-red-900/20 pt-2">Poll ends in {(pollDays * 24) + pollHours} hours</div>
                                    {selectedImagePath && (
                            ...
                            {activeTab === 'prospect' && (
                                <div>
                                    <div className="mb-3">
                                        <img src={judgeImage} alt='Judging' className="w-full rounded-lg border border-red-900/40 px-1" />
                                    </div>
